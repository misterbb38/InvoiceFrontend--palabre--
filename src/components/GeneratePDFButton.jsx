...
function GeneratePDFButton({ invoice, currency }) {
  ...
  const generatePDF = () => {
    ...
    const addFooter = () => {
      ...
      doc.text('Domiciliation : Pompidou x Raffanel, Immeuble Yoro LAM SENEGAL', 30, footerY + 12)
    ...
    doc.moveTo(103, currentY + 27)  // Position de départ de la ligne
    ...
    doc.setFontSize(8)
    doc.text(`Sacré Cœur 3, Villa n° 8974, Code postal : 11000, Dakar`, 20, currentY + 5)
    doc.text(`Amadou Kéthiel KEBE - Traducteur Freelance - anglais, français, portugais`, 20, currentY + 10)
...
doc.moveTo(163, currentY + 7)  // Position de départ de la ligne
   .lineTo(200, currentY + 7)  // Position finale de la ligne
   .stroke();  // Dessine la ligne

// Ajout du texte 'MIAAE'
doc.text(
  `MIAAE ${invoice.invoiceNumber}`,
  175,
  currentY + 15
);

// Ajout de la ligne sous le texte 'MIAAE'
doc.moveTo(163, currentY + 17)  // Position de départ de la ligne
  ...
  let currentCategory = ''
    ...
    sortedItems.forEach((item, index) => {
      ...
      if (item.category !== currentCategory) {
        ...
        doc.setFontSize(8)
        doc.text(`${currentCategory} des documents intitulés :`, 105, currentY + 4, { align: 'center' }) // Centrer le texte de la catégorie
      ...
      const itemTextHeight = doc.splitTextToSize(item.description, 50).length * 2.5 // Hauteur des lignes de texte réduite
      ...
      doc.setFillColor(isEven ? 224 : 200, isEven ? 224 : 230, isEven ? 224 : 201); // Gris clair moyen pour pairs, vert menthe pour impairs
      ...
      doc.text(doc.splitTextToSize(item.description, 60), 42, textY, { baseline: 'middle' })
      doc.text(item.quantity.toString(), 112, textY, { baseline: 'middle' })
    
      const formattedPrice = formatNumber(item.price.toFixed(0))
      const formattedTotal = formatNumber(item.total.toFixed(0))
    
      doc.text(`${formattedPrice} ${currency}`, 132, textY, { baseline: 'middle' })
      doc.text(`${formattedTotal} ${currency}`, 162, textY, { baseline: 'middle' })
...
function nombreEnLettres(nombre) {
  ...
  if (nombre < 20) {
    ...
    return (cent > 1 ? unite[cent] + ' ' : '') + 'cent' + (reste > 0 ? ' ' + nombreEnLettres(reste) : '');
  } else if (nombre < 1000000) {
    let reste = nombre % 1000;
    let mille = Math.floor(nombre / 1000);
    return (mille > 1 ? nombreEnLettres(mille) + ' mille' : 'mille') + (reste > 0 ? ' ' + nombreEnLettres(reste) : '');
  }
...
function convertirSommeEnLettres(montant) {
  ...
  let centimesEnLettres = centimes > 0 ? `et ${nombreEnLettres(centimes)} centimes` : '';
...
doc.text(`MONTANT TOTAL ${formatNumber(totalAmount.toFixed(0))} ${originalCurrency}`, 130, currentY);
